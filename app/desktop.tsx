...
// Modified: Accepts onNavigate to trigger animations
            ...
            {items.map((item, idx) => (
              ...
              >
                <a
                  ...
                  onClick={(e) => {
                    // Intercept for Projects/Notes
                    if ((item.href === "/projects" || item.href === "/notes") && onNavigate) {
                        e.preventDefault();
...
function IconContainer({
  ...
  return (
    <a 
      ...
      onClick={(e) => {
        // Intercept for Projects/Notes
        if ((href === "/projects" || href === "/notes") && onNavigate) {
            e.preventDefault();
            onNavigate(href);
        }
        else if (href === "#") {
...
export const StarsBackground = ({ className, starColor = "255, 255, 255" }: { className?: string; starColor?: string }) => {
...
export const ShootingStars = ({ minDelay = 50, maxDelay = 200, className }: { minDelay?: number, maxDelay?: number, className?: string }) => {
...
export const BlurFade = ({ children, delay = 0, className, yOffset = 20 }: { children: React.ReactNode; delay?: number; className?: string; yOffset?: number; }) => (
...
// Modified: Accepts onClick to support animations
export const TopNavButton = ({ text, href, newTab = false, onClick }: { text: string; href: string; newTab?: boolean; onClick?: (e: React.MouseEvent) => void }) => (
  <a 
    href={href}
    target={newTab ? "_blank" : undefined}
    rel={newTab ? "noopener noreferrer" : undefined}
    onClick={onClick}
    className="group relative px-6 py-2 rounded-full bg-neutral-900/50 border border-white/10 hover:border-white/30 text-sm font-medium text-neutral-300 transition-all duration-300 hover:bg-white/10 backdrop-blur-sm overflow-hidden cursor-pointer"
  >
...
// Modified: Accepts onNavigate handler
export const HeaderButtons = ({ onNavigate }: { onNavigate: (path: string) => void }) => (
  <motion.div 
...
const ShootingStarsPageDesktop = () => {
  ...
  const [typewriterDelay, setTypewriterDelay] = useState(1.8);
  ...
  const toolsPlatforms = [
    "Git", "Scikit", "MongoDB", "PostgreSQL", "Vercel", "Railway", "Ngrok", 
 ...
 const dockItems = [
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-white font-sans relative w-full overflow-x-hidden pb-7">
      ...
      <motion.div
        className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 via-violet-500 to-pink-500 origin-left z-[60]"
        style={{ scaleX }}
      />

      {/* Intro Overlay (Initial Load) */}
      <AnimatePresence mode="wait">
        {showIntro && <IntroOverlay />}
      </AnimatePresence>

      {/* Exit Overlay (Fast 0.1s Curtain on Navigate) */}
      <AnimatePresence>
        {isExiting && (
          <motion.div
            initial={{ y: "100%" }}
      ...
      <section className="h-screen w-full flex flex-col items-center justify-center relative z-10 p-4">
        {/* NEW HEADER BUTTONS WITH NAVIGATION HANDLER */}
        <HeaderButtons onNavigate={handleNavigation} />
      ...
      <section id="skills" className="min-h-screen w-full flex flex-col items-center justify-center relative z-10 py-20 px-4 md:px-10 bg-transparent">
        <div className="max-w-6xl w-full mx-auto space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Core Technologies */}
            <BlurFade delay={0.3} className="relative p-6 border border-white/10 rounded-2xl bg-neutral-800/30 hover:bg-neutral-800/50 transition-colors hover:border-purple-500/40">
            ...
            {/* Tools & Platforms */}
            <BlurFade delay={0.4} className="relative p-6 border border-white/10 rounded-2xl bg-neutral-800/30 hover:bg-neutral-800/50 transition-colors hover:border-purple-500/40">
            ...
            {/* Soft Skills */}
            <BlurFade delay={0.5} className="relative p-6 border border-white/10 rounded-2xl bg-neutral-800/30 hover:bg-neutral-800/50 transition-colors hover:border-purple-500/40">
            ...
            <div className="flex flex-col space-y-6 text-left order-2 md:order-1">
                ...
                <BlurFade delay={0.3}>
                    <p className="text-lg md:text-xl text-neutral-300 leading-relaxed font-medium">
                        I'm a <span className="text-white font-semibold">{age} y/o</span> studying at <span className="text-white font-semibold">Manipal University Jaipur</span>.
                    </p>
                </BlurFade>
      ...
      <div className="fixed bottom-5 left-1/2 transform -translate-x-1/2 z-50">
        <FloatingDock 
          ...
          onNavigate={handleNavigation} // Pass the new handler here
        />
      </div>
    </div>
  );
...
export default ShootingStarsPageDesktop;