"use client";
import React, { useState, useEffect, useRef, useCallback, useMemo, ComponentPropsWithoutRef } from "react";
...
// =========================================
// PIXEL IMAGE COMPONENT (FIXED FOR HYDRATION)
// =========================================
...
const TypewriterEffectSmooth = ({ 
  ...
  className, 
  cursorClassName,
  delay = 0 // Added default delay
}: { 
  words: { text: string; className?: string }[]; 
  className?: string; 
  cursorClassName?: string;
  delay?: number; // Added delay prop type
}) => {
  const renderWords = () => (
    <div>
      {words.map((word, idx) => (
  ...
  return (
    <div className={cn("flex space-x-1 my-6", className)}>
      <motion.div 
        ...
        viewport={{ once: true }} // FIX: Added this so it doesn't restart annoyingly
        transition={{ 
          duration: 2, 
          ease: "linear", 
          delay: delay // Used the delay prop here
        }}
      >
